...
lapply(packages, library, character.only=TRUE) #lsmeans and Rmisc may need download from CRAN

#######If there are problems installing these packages from the above code #####
#install.packages('lsmeans', repos='http://cran.us.r-project.org', dependencies = TRUE)
#install.packages('Rmisc', repos='http://cran.us.r-project.org', dependencies = TRUE)
...
#write.xlsx(turcicum_flutriafol, "Flutriafol relative growth data.xlsx")
```
...
```{r}
#LL.2
LL.2_modelfit<-drm(formula = turcicum_flutriafol$relative_growth ~ turcicum_flutriafol$ppm, fct = LL.2(),type="continuous")
...
# LL.3
LL.3_modelfit<-drm(formula = turcicum_flutriafol$relative_growth ~ turcicum_flutriafol$ppm, fct = LL.3(),type="continuous")
...
# LL.4
LL.4a_modelfit<-drm(formula = turcicum_flutriafol$relative_growth ~ turcicum_flutriafol$ppm, fct = LL.4(),type="continuous")
...
#BC.4
BC.4_modelfit <- drm(formula = turcicum_flutriafol$relative_growth ~ turcicum_flutriafol$ppm, fct = BC.4(),type="continuous")
...
#CRS.4c_modelfit <- drm(formula = turcicum_flutriafol$relative_growth ~ turcicum_flutriafol$ppm, fct = CRS.4c(),type="continuous")
...
#W2.4
W2.4_modelfit <- drm(formula = turcicum_flutriafol$relative_growth ~ turcicum_flutriafol$ppm, fct = W2.4(),type="continuous")
...
```{r}
mselect(LL.2_modelfit, func.list, icfct=AIC)
mselect(LL.2_modelfit, func.list, icfct=BIC)
...
EC50s_by_state <- myEC50_and_UKYEC50_commpare %>%
  filter(!is.na(Absolute.EC50.estimate_ll4))%>%
  select(., select = -c(isolatexbatch)) %>%
  group_by(state) %>%
  mutate(Mean_abs_ec50_ll4 = mean(Absolute.EC50.estimate_ll4), # not working because these now have either a "_ll4" or "w24" suffix
         sd_abs_ec50_ll4 = sd(Absolute.EC50.estimate_ll4),
         Mean_rel_ec50_ll4 = mean(Relative.EC50.estimate_ll4),
         sd_rel_ec50_ll4 = sd(Relative.EC50.estimate_ll4),
         
         Mean_UKY_ec50 = mean(mean_EC50),
         sd_UKY_ec50 = sd(mean_EC50),
         N_tested = n()) %>%
  select(., select = -c(Absolute.EC50.estimate_ll4, Absolute.EC50.estimate_w24, Relative.EC50.estimate_w24, Relative.EC50.estimate_ll4,  mean_EC50)) %>%
  filter(! duplicated(state)) %>%
  select(state, N_tested, Mean_abs_ec50_ll4, sd_abs_ec50_ll4, Mean_rel_ec50_ll4, sd_rel_ec50_ll4, Mean_UKY_ec50, sd_UKY_ec50)
...
LL4_abs_EC50_hist <- ggplot(myEC50_and_UKYEC50_commpare, aes(x=Absolute.EC50.estimate_ll4, fill=state)) +
...
LL4_rel_EC50_hist <- ggplot(myEC50_and_UKYEC50_commpare, aes(x=Relative.EC50.estimate_ll4, fill=state)) +
  geom_histogram(binwidth = 0.5) + #everything grouped within 0.5 ppm EC50 of another
  theme_gray() +
  theme(axis.text.x = element_text(size = 9, face = "bold", angle=45, hjust=1, family = "serif"),
        axis.text.y = element_text(size = 20, face = "bold", family = "serif"),
...
```{r, eval=FALSE, fig.asp=0.8, fig.width=20, out.width="100%", dpi=600}
Absolute_ec50_box <- ggplot(myEC50_and_UKYEC50_commpare, aes(x=state, y=Absolute.EC50.estimate_ll4, fill = state)) +
  geom_boxplot() +
  coord_flip() +
theme_gray() +
  theme(axis.text.x = element_text(size = 9, face = "bold", angle=45, hjust=1, family = "serif"),
...
Relative_ec50_box <- ggplot(myEC50_and_UKYEC50_commpare, aes(x=state, y=Relative.EC50.estimate_ll4, fill = state)) +
  geom_boxplot() +
  coord_flip() +
theme_gray() +
  theme(axis.text.x = element_text(size = 9, face = "bold", angle=45, hjust=1, family = "serif"),
...
t.test(myEC50_and_UKYEC50_commpare$Absolute.EC50.estimate_ll4, myEC50_and_UKYEC50_commpare$Relative.EC50.estimate_ll4)

# Absolute vs UKY EC50 estimation method, significant differences as expected
t.test(myEC50_and_UKYEC50_commpare$Absolute.EC50.estimate_ll4, myEC50_and_UKYEC50_commpare$mean_EC50)

# Relative vs UKY EC50 estimation method, no significant differences. These methods peroduced similar results.
t.test(myEC50_and_UKYEC50_commpare$Relative.EC50.estimate_ll4, myEC50_and_UKYEC50_commpare$mean_EC50)
...
ET20KY6_KY8_reproduce_ll4 <- EC50_data_batches %>%
  ...
  summarize(mean_ll4 = mean(as.numeric(Absolute.EC50.estimate_ll4), na.rm=TRUE),
            sd_ll4 = sd(as.numeric(Absolute.EC50.estimate_ll4), na.rm=TRUE),
            plus3_stdv_ll4 = mean(as.numeric(Absolute.EC50.estimate_ll4), na.rm=TRUE)+(3*sd(Absolute.EC50.estimate_ll4, na.rm=TRUE)),
            minus3_stdv_ll4 = mean(as.numeric(Absolute.EC50.estimate_ll4), na.rm=TRUE)-(3*sd(Absolute.EC50.estimate_ll4)))
